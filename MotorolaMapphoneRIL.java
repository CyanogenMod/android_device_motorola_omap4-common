...
/**
 * Mapphone RIL for the Motorola OMAP4 family.
 * This RIL is univerisal meaning it supports CDMA and GSM radio.
 * Handles most GSM and CDMA cases.
 * {@hide}
 */
public class MotorolaMapphoneRIL extends RIL implements CommandsInterface {
    ...
    public MotorolaMapphoneRIL(Context context, int networkMode, int cdmaSubscription) {
    ...
    processUnsolicited (Parcel p) {
        ...
        switch(response) {
            case RIL_UNSOL_RIL_CONNECTED: // Fix for NV/RUIM setting on CDMA SIM devices
                // skip getcdmascriptionsource as if it is handled in the ril binary
                ret = responseInts(p);
                setRadioPower(false, null);
                setPreferredNetworkType(mPreferredNetworkType, null);
                int cdmaSubscription = Settings.Global.getInt(mContext.getContentResolver(),
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
            ...
            case RIL_REQUEST_SIGNAL_STRENGTH: ret =  responseSignalStrength(p); break;
                // modification start
                // prevent exceptions from happenimg because the null value is null or a hexadecimel
                // so convert if it is not null
            case RIL_REQUEST_VOICE_REGISTRATION_STATE: ret =  responseVoiceDataRegistrationState(p);
                break;
            case RIL_REQUEST_DATA_REGISTRATION_STATE: ret =  responseVoiceDataRegistrationState(p);
                break;
                // this fixes bogus values the modem creates
                // sometimes the  ril may print out (null,null,31000)
                // this problemaic as eHRPD is always reported as network type unknown
    ...
    // CDMA FIXES, this fixes  bogus values in nv/sim on Mapphone cdma family
    private Object
    operatorCheck(Parcel p) {
        ...
        for(int i=0; i<response.length; i++){
            if (response[i]!= null){
                ...
                } else if (response[i].equals("31000")|| response[i].equals("11111") ||
                        response[i].equals("123456") || response[i].equals("31099") ||
                        (response[i].equals("") && !isGSM)){
