...
#define LOG_TAG "audio_hw_secondary"
#define LOG_NDEBUG 0
...
static int start_output_stream(struct omap_stream_out *out)
{
    ...
    ALOGI("STARGO: opening card: %d, port: %d for device: %x, rate: %d, channels: %d",
...
/* STARGO */
        ...
        for (i = 0; i < out_frames; i++) {
            ...
            m = (((int16_t*)buf)[i*2] + ((int16_t*)buf)[(i*2)+1]) / 2;
            ((int16_t*)buf)[i] = m & 0xffff;
        }
        /* STARGO */
