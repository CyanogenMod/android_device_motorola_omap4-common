...
#define LOG_TAG "audio_hw_moto_oss"
#define LOG_NDEBUG 0
...
#define MIXER_HEADSET_PLAYBACK_VOLUME       "STDac Volume"
#define MIXER_HANDSFREE_PLAYBACK_VOLUME     "STDac Volume"
...
#define MIXER_HS_LEFT_PLAYBACK              "HSLDAC Switch"
#define MIXER_HS_RIGHT_PLAYBACK             "HSRDAC Switch"
#define MIXER_HF_LEFT_PLAYBACK              "Handsfree Left Playback"
#define MIXER_HF_RIGHT_PLAYBACK             "Handsfree Right Playback"
#define MIXER_EARPHONE_ENABLE_SWITCH        "EPCDC Switch"
...
/*
 * 0: Multimedia
 * 1: McBSP3-Codec cpcap
 * 2: Modem-Codec cpcap
 * 3: FMAudio cpcap
 * 4: Modem-BT cpcap
 * 5: McBSP3-BT cpcap
 * 6: SPDIF PCM Playback
 * 7: McBSP3-BP BPVoice-7
 * 8: FM Playback cpcap
 */
#define PORT_MM 0
...
/* sampling rate when using BT SCO port */
#define SCO_SAMPLING_RATE VX_NB_SAMPLING_RATE
...
static int start_output_stream(struct omap_stream_out *out)
{
    ...
    if (out->device & AUDIO_DEVICE_OUT_ALL_SCO) {
        out->config.rate = SCO_SAMPLING_RATE; /* STARGO: MM_FULL_POWER_SAMPLING_RATE */
        out->config.channels = 1; /* STARGO */
        port = PORT_BT_SCO;
    } else {
    ...
    ALOGI("opening card (out): %d, port: %d for device: %x, rate: %d, channels: %d",
      card, port, out->device, out->config.rate, out->config.channels);
...
/* STARGO */
        ...
        out->resampler->resample_from_input(out->resampler,
                ...
                &out_frames);
        buf = out->buffer;
        /* STARGO: Poor mans channel dropping */
        for (i = 1; i < (out_frames/2); i++) {
            ((int16_t*)buf)[i] = ((int16_t*)buf)[i*2];
        }
        frame_size /= 2;
        /* STARGO */
...
static int start_input_stream(struct omap_stream_in *in)
{
    ...
    ALOGI("opening card (in): %d, port: %d for device: %x, rate: %d, channels: %d",
      card, device, in->device, in->config.rate, in->config.channels);
...
struct audio_module HAL_MODULE_INFO_SYM = {
    .common = {
        ...
        .name = "Motorola OSS audio HW HAL",
        .author = "CyanogenMod",
