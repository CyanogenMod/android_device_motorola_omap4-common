...
public class DeviceSettings extends PreferenceActivity implements OnPreferenceChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        } else if (SystemProperties.get("ro.telephony.ril.v3","").equals("writeaidonly,signalstrength")) {
			SystemProperties.set("persist.sys.gsm_fix", "1");
        }
        mGSMSignalStrengthPref = (CheckBoxPreference) getPreferenceScreen().findPreference(KEY_GSM_SIGNALSTRENGTH);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if(preference == mGSMSignalStrengthPref) {
            Log.d(TAG,"Setting persist.sys.gsm_fix to "+(mGSMSignalStrengthPref.isChecked() ? "1" : "0"));
