...
static int responseDataCallListV6(Parcel &p, void *response, size_t responselen)
{
	...
	if (p_cur[i].ifname && p_cur[i].gateways) {
	    for (gws = p_cur[i].gateways; ; gws = NULL) {
	        struct in_addr ia;

	    	gw = strtok_r(gws, " ", &sptr);
		if (gw == NULL)
		    break;

		if (inet_aton(gw, &ia) == 0) {
	            RLOGI("Omap4: %s is not a valid IPv4 address!", gw);
		    continue;
		}

	        RLOGI("Omap4: found gateway %s on %s", gw, p_cur[i].ifname);

                memset(cmd, 0, sizeof(cmd));
                snprintf(cmd, sizeof(cmd)-1, "%s:%s", p_cur[i].ifname, gw);
                RLOGI(cmd);
		property_set("ril.motoril.route", cmd);
	    }
	}
